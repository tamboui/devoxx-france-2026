...
public class adoc2reveal implements Callable<Integer> {
    ...
    @Option(names={"--serve"}, description="Serve the output directory via built-in HTTP server (implies --watch)")
    ...
    public Integer call() throws Exception {
        ...
        if (serve) {
            ...
            Path serveDir = file.getAbsoluteFile().getParentFile().toPath();
            var server = SimpleFileServer.createFileServer(
                    new InetSocketAddress(port),
                    serveDir,
                    SimpleFileServer.OutputLevel.NONE);
            server.start();
            String outputName = file.getName().replaceFirst("\\.adoc$", ".html");
            System.out.printf("Serving at http://localhost:%d/%s%n", port, outputName);
        }
